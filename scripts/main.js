// HUM 공통 동작: 제품 개폐 → 영상/조합 → 인트로 → 스크롤 → 커서
// 문구와 이미지는 index.html, 시각 스타일은 styles/main.css를 수정하세요.
'use strict';
// ── 01. 제품 뒷면 열기/닫기 ──
const views = {
  open: {
...
const pairs = fragments.flatMap((first, i) =>
  fragments.slice(i + 1).map((second) => first + second)
);
const playSafely = (video) =>
  video.play().catch(() => {
    /* Posters remain visible when autoplay is blocked. */
  });
function syncPlayback() {
  if (document.hidden || introDialog.open) {
    heroVideo.pause();
    labVideo.pause();
    return;
  }
  if (heroWantsMotion && heroInView) playSafely(heroVideo);
  else heroVideo.pause();
  if (labWantsMotion && labInView && selected.length === 2) playSafely(labVideo);
  else labVideo.pause();
}
function updatePlaybackLabels() {
  heroToggle.innerHTML = heroVideo.paused
    ? '모션 재생 <span aria-hidden="true">▷</span>'
    : '모션 멈추기 <span aria-hidden="true">Ⅱ</span>';
  heroToggle.setAttribute(
    'aria-label',
    heroVideo.paused ? '첫 화면 모션 재생' : '첫 화면 모션 일시정지'
  );
  labToggle.textContent = labVideo.paused ? '▷' : 'Ⅱ';
  labToggle.setAttribute('aria-label', labVideo.paused ? '조합 영상 재생' : '조합 영상 일시정지');
}
heroToggle.addEventListener('click', () => {
  heroWantsMotion = heroVideo.paused;
  heroInView = true;
  syncPlayback();
});
labToggle.addEventListener('click', () => {
  labWantsMotion = labVideo.paused;
  labInView = true;
  syncPlayback();
});
[heroVideo, labVideo].forEach((video) => {
  video.addEventListener('play', updatePlaybackLabels);
...
function choosePair(next) {
  ...
  if (selected.length !== 2) {
    labVideo.pause();
    labVideo.hidden = true;
    loading.hidden = false;
    loading.textContent = selected.length ? '두 번째 조각을 선택하세요' : '두 조각을 선택해 주세요';
    document.getElementById('combination-title').textContent = selected.length
  ...
  document.getElementById('combination-counter').textContent =
    String(pairs.indexOf(currentPair) + 1).padStart(2, '0') + ' / 15';
  document.getElementById('selection-status').textContent =
    labels.join(' + ') + ' 조합을 감상해 보세요.';
  loading.hidden = false;
  loading.textContent = '새로운 형태를 불러오는 중';
  labVideo.hidden = false;
...
document.getElementById('motion-speed').addEventListener('input', (event) => {
  ...
  labVideo.playbackRate = rate;
  document.getElementById('speed-label').textContent =
    (Number.isInteger(rate) ? rate.toFixed(1) : String(rate)) + '×';
});
labVideo.addEventListener('loadeddata', () => {
  loading.hidden = true;
});
labVideo.addEventListener('error', () => {
  loading.hidden = false;
  loading.textContent = '영상을 불러오지 못했어요. 다른 조합을 선택해 주세요.';
});
labVideo.addEventListener('timeupdate', () => {
  document.getElementById('video-progress').style.width =
    (Number.isFinite(labVideo.duration) ? (100 * labVideo.currentTime) / labVideo.duration : 0) +
    '%';
});
// ── 03. 첫 방문 인트로 ──
...
function finishIntro() {
  ...
  introClosing = true;
  introDialog.classList.add('is-leaving');
  introFilm.pause();
  window.setTimeout(
    () => {
      introDialog.close();
...
introFilm.addEventListener('timeupdate', () => {
  document.getElementById('intro-progress-fill').style.width =
    (Number.isFinite(introFilm.duration) ? (100 * introFilm.currentTime) / introFilm.duration : 0) +
    '%';
});
...
function nextHero() {
  if (!heroBag.length) {
    ...
    for (let i = heroBag.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [heroBag[i], heroBag[j]] = [heroBag[j], heroBag[i]];
    }
  }
...
mediaObserver.observe(labVideo);
document.addEventListener('visibilitychange', syncPlayback);
reducedMotion.addEventListener('change', () => {
  heroWantsMotion = !reducedMotion.matches;
  labWantsMotion = !reducedMotion.matches;
...
if (!reducedMotion.matches) {
  ...
  const reveals = document.querySelectorAll(
    '.story-layout,.device-layout,.interaction,.section-heading,.city-layout,.company-layout,.reveal'
  );
...
['device', 'archive', 'team'].forEach((id) =>
  navigationObserver.observe(document.getElementById(id))
);
...
if (window.matchMedia('(hover: hover) and (pointer: fine)').matches) {
  tilt.addEventListener('pointermove', (event) => {
    ...
    tilt.style.transform = `perspective(900px) rotateX(${-y * 6}deg) rotateY(${x * 6}deg)`;
  });
...
document.addEventListener(
  ...
  (event) => {
    ...
    cursor.classList.toggle(
      'is-link',
      Boolean(event.target.closest('a,button,summary,input[type=range]'))
    );
...
document.addEventListener(
  ...
  (event) => {
    ...
    if (event.target.closest('.device,.company-section,.team-section,.closing,.story,.city'))
      wave.classList.add('is-light');
